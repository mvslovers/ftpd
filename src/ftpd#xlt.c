/*
** FTPD EBCDIC <-> ASCII Translation Tables
**
** Two codepages from HTTPD (httpxlat.c):
**   IBM1047 — active default for FTP protocol and UFS content
**   CP037   — for MVS dataset content (ASCII-mode transfers)
**
** NL/LF mapping (both codepages, modified for mvslovers ecosystem):
**   ASCII LF  (0x0A) -> EBCDIC NEL (0x15)
**   EBCDIC NEL(0x15) -> ASCII  LF  (0x0A)
**   EBCDIC LF (0x25) -> ASCII  NEL (0x85)  (unchanged)
**
** c2asm370 maps '\n' to EBCDIC NEL (0x15).  The override ensures
** roundtrip fidelity for all C-generated text.
*/
#include "ftpd#xlt.h"
...
static unsigned char ibm1047_atoe[256] = {
    ...
    /* 0x58-0x5F */ 0xE7, 0xE8, 0xE9, 0xAD, 0xE0, 0xBD, 0x5F, 0x6D,
...
static unsigned char ibm1047_etoa[256] = {
    ...
    /* 0x40-0x47 */ 0x20, 0xA0, 0xE2, 0xE4, 0xE0, 0xE1, 0xE3, 0xE5,
    /* 0x48-0x4F */ 0xE7, 0xF1, 0xA2, 0x2E, 0x3C, 0x28, 0x2B, 0x7C,
    /* 0x50-0x57 */ 0x26, 0xE9, 0xEA, 0xEB, 0xE8, 0xED, 0xEE, 0xEF,
    /* 0x58-0x5F */ 0xEC, 0xDF, 0x21, 0x24, 0x2A, 0x29, 0x3B, 0x5E,
    /* 0x60-0x67 */ 0x2D, 0x2F, 0xC2, 0xC4, 0xC0, 0xC1, 0xC3, 0xC5,
    /* 0x68-0x6F */ 0xC7, 0xD1, 0xA6, 0x2C, 0x25, 0x5F, 0x3E, 0x3F,
    /* 0x70-0x77 */ 0xF8, 0xC9, 0xCA, 0xCB, 0xC8, 0xCD, 0xCE, 0xCF,
    /* 0x78-0x7F */ 0xCC, 0x60, 0x3A, 0x23, 0x40, 0x27, 0x3D, 0x22,
    /* 0x80-0x87 */ 0xD8, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67,
    /* 0x88-0x8F */ 0x68, 0x69, 0xAB, 0xBB, 0xF0, 0xFD, 0xFE, 0xB1,
    /* 0x90-0x97 */ 0xB0, 0x6A, 0x6B, 0x6C, 0x6D, 0x6E, 0x6F, 0x70,
    /* 0x98-0x9F */ 0x71, 0x72, 0xAA, 0xBA, 0xE6, 0xB8, 0xC6, 0xA4,
    /* 0xA0-0xA7 */ 0xB5, 0x7E, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78,
    /* 0xA8-0xAF */ 0x79, 0x7A, 0xA1, 0xBF, 0xD0, 0x5B, 0xDE, 0xAE,
    /* 0xB0-0xB7 */ 0xAC, 0xA3, 0xA5, 0xB7, 0xA9, 0xA7, 0xB6, 0xBC,
    /* 0xB8-0xBF */ 0xBD, 0xBE, 0xDD, 0xA8, 0xAF, 0x5D, 0xB4, 0xD7,
    /* 0xC0-0xC7 */ 0x7B, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47,
    /* 0xC8-0xCF */ 0x48, 0x49, 0xAD, 0xF4, 0xF6, 0xF2, 0xF3, 0xF5,
    /* 0xD0-0xD7 */ 0x7D, 0x4A, 0x4B, 0x4C, 0x4D, 0x4E, 0x4F, 0x50,
    /* 0xD8-0xDF */ 0x51, 0x52, 0xB9, 0xFB, 0xFC, 0xF9, 0xFA, 0xFF,
...
static unsigned char cp037_atoe[256] = {
    ...
    /* 0x48-0x4F */ 0xC8, 0xC9, 0xD1, 0xD2, 0xD3, 0xD4, 0xD5, 0xD6,
    /* 0x50-0x57 */ 0xD7, 0xD8, 0xD9, 0xE2, 0xE3, 0xE4, 0xE5, 0xE6,
    /* 0x58-0x5F */ 0xE7, 0xE8, 0xE9, 0xBA, 0xE0, 0xBB, 0xB0, 0x6D,
    /* 0x60-0x67 */ 0x79, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x87,
    /* 0x68-0x6F */ 0x88, 0x89, 0x91, 0x92, 0x93, 0x94, 0x95, 0x96,
    /* 0x70-0x77 */ 0x97, 0x98, 0x99, 0xA2, 0xA3, 0xA4, 0xA5, 0xA6,
    /* 0x78-0x7F */ 0xA7, 0xA8, 0xA9, 0xC0, 0x4F, 0xD0, 0xA1, 0x07,
...
static unsigned char cp037_etoa[256] = {
    ...
    /* 0x48-0x4F */ 0xE7, 0xF1, 0xA2, 0x2E, 0x3C, 0x28, 0x2B, 0x7C,
    /* 0x50-0x57 */ 0x26, 0xE9, 0xEA, 0xEB, 0xE8, 0xED, 0xEE, 0xEF,
    /* 0x58-0x5F */ 0xEC, 0xDF, 0x21, 0x24, 0x2A, 0x29, 0x3B, 0xAC,
    /* 0x60-0x67 */ 0x2D, 0x2F, 0xC2, 0xC4, 0xC0, 0xC1, 0xC3, 0xC5,
    /* 0x68-0x6F */ 0xC7, 0xD1, 0xA6, 0x2C, 0x25, 0x5F, 0x3E, 0x3F,
    /* 0x70-0x77 */ 0xF8, 0xC9, 0xCA, 0xCB, 0xC8, 0xCD, 0xCE, 0xCF,
    /* 0x78-0x7F */ 0xCC, 0x60, 0x3A, 0x23, 0x40, 0x27, 0x3D, 0x22,
    ...
    /* 0x98-0x9F */ 0x71, 0x72, 0xAA, 0xBA, 0xE6, 0xB8, 0xC6, 0xA4,
    /* 0xA0-0xA7 */ 0xB5, 0x7E, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78,
    /* 0xA8-0xAF */ 0x79, 0x7A, 0xA1, 0xBF, 0xD0, 0xDD, 0xDE, 0xAE,
    /* 0xB0-0xB7 */ 0x5E, 0xA3, 0xA5, 0xB7, 0xA9, 0xA7, 0xB6, 0xBC,
    /* 0xB8-0xBF */ 0xBD, 0xBE, 0x5B, 0x5D, 0xAF, 0xA8, 0xB4, 0xD7,
    /* 0xC0-0xC7 */ 0x7B, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47,
    /* 0xC8-0xCF */ 0x48, 0x49, 0xAD, 0xF4, 0xF6, 0xF2, 0xF3, 0xF5,
    /* 0xD0-0xD7 */ 0x7D, 0x4A, 0x4B, 0x4C, 0x4D, 0x4E, 0x4F, 0x50,
    /* 0xD8-0xDF */ 0x51, 0x52, 0xB9, 0xFB, 0xFC, 0xF9, 0xFA, 0xFF,
    /* 0xE0-0xE7 */ 0x5C, 0xF7, 0x53, 0x54, 0x55, 0x56, 0x57, 0x58,
    /* 0xE8-0xEF */ 0x59, 0x5A, 0xB2, 0xD4, 0xD6, 0xD2, 0xD3, 0xD5,
    /* 0xF0-0xF7 */ 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37,
    /* 0xF8-0xFF */ 0x38, 0x39, 0xB3, 0xDB, 0xDC, 0xD9, 0xDA, 0x9F
};
...
/* In-place ASCII -> EBCDIC (IBM-1047) */
void
ftpd_xlat_a2e(unsigned char *buf, int len)
{
    int i;
    for (i = 0; i < len; i++)
        buf[i] = ibm1047_atoe[buf[i]];
}

/* In-place EBCDIC -> ASCII (IBM-1047) */
void
ftpd_xlat_e2a(unsigned char *buf, int len)
{
    int i;
    for (i = 0; i < len; i++)
        buf[i] = ibm1047_etoa[buf[i]];
}

/* In-place ASCII -> EBCDIC (CP037 — MVS datasets) */
void
ftpd_xlat_mvs_a2e(unsigned char *buf, int len)
{
    int i;
    for (i = 0; i < len; i++)
        buf[i] = cp037_atoe[buf[i]];
}

/* In-place EBCDIC -> ASCII (CP037 — MVS datasets) */
void
ftpd_xlat_mvs_e2a(unsigned char *buf, int len)
{
    int i;
    for (i = 0; i < len; i++)
        buf[i] = cp037_etoa[buf[i]];
}
