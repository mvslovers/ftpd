...
| `552 ... out of space on the data set` on an upload | The data set could not grow: the space it was allocated is used up. `552` is permanent on purpose — retrying the same upload into the same allocation cannot succeed. Give the transfer more space *before* the next attempt: raise `DEFPRIMARY`/`DEFSECONDARY`/`DEFSPACETYPE` in `FTPDPRM`, or have the client send `SITE CYLINDERS PRIMARY=10 SECONDARY=10`. If FTPD created the data set for this upload it scratches it again, so the retry starts from a clean allocation. Needs libc370 1.0.6 or later; on an older runtime the same upload ends in an `IEC031I` x37 ABEND instead, and leaves a partial data set that nothing can delete until the started task is restarted |
