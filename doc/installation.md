...
| `552 ... out of space on the data set (ABEND S D37/B37)` on an upload | The data set could not grow: the space it was allocated is used up. `552` is permanent on purpose — retrying the same upload into the same allocation cannot succeed. Give the transfer more space *before* the first attempt: raise `DEFPRIMARY`/`DEFSECONDARY`/`DEFSPACETYPE` in `FTPDPRM`, or have the client send `SITE CYLINDERS PRIMARY=10 SECONDARY=10`. **The partial data set is left behind and cannot be deleted until FTPD is restarted** — the failed CLOSE keeps it allocated to the FTPD address space, so `DELE` answers `IDCAMS rc=8` for any client. Delete it from TSO or a batch job, or restart the started task (mvslovers/libc370#168) |
